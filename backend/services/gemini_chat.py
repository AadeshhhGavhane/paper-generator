...
def retry_with_backoff(func, max_retries=3, base_delay=1, max_delay=60):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if any(keyword in error_msg.lower() for keyword in ['overloaded', '503', 'unavailable', 'rate limit', 'quota']):
                if attempt < max_retries - 1:  # Don't sleep on the last attempt
                    # Exponential backoff with jitter
                    delay = min(base_delay * (2 ** attempt) + random.uniform(0, 1), max_delay)
                    print(f"API overloaded, retrying in {delay:.1f} seconds...")
                    time.sleep(delay)
                    continue
            ...
            if attempt == max_retries - 1:
                ...
                if 'overloaded' in error_msg.lower() or '503' in error_msg:
                    raise RuntimeError("🚫 Google Gemini API is currently overloaded. Please try again in a few minutes, or use Groq instead.")
                elif 'quota' in error_msg.lower():
                    raise RuntimeError("🚫 API quota exceeded. Please check your Google AI API limits or try Groq instead.")
                elif 'invalid' in error_msg.lower() and 'key' in error_msg.lower():
                    raise RuntimeError("🚫 Invalid Google API key. Please check your GOOGLE_API_KEY in the .env file.")
                else:
